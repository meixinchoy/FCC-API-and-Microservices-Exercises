...
var createAndSavePerson = function (done) {
  var person1 = new Person({ name: "Janek", age: 64, favoriteFoods: ["egg", "bacon", "burrito"] });
...
var arrayOfPeople = [
  ...
  { name: "Ren", age: 6 },
  { name: "Emmy", age: 8, favoriteFoods: ["Pizza", "burrito"] }
];
...
// Find all that satisfies. `Model.find()` accepts a **query** document (a JSON
...
// When saving a document, mongodb automatically add the field `_id`, and set
// it to a unique alphanumeric key.`moongose` provides a dedicated method for it. 
...
// `findByIdAndRemove()` or `findOneAndRemove()`, similar to the update methods
var removeById = function (personId, done) {
  Person.findByIdAndRemove(personId, (err, removedPpl) => {
    if (err) return console.log(err);
    done(null, removedPpl);
  })
};
...
Person.find({ favoriteFoods: foodToSearch })
  ...
  .select({ age: 0 })
  .sort({ name: 1 })
  .exec(function (err, pplFound) {
    if (err) console.log(err);
    console.log(pplFound);
  })
